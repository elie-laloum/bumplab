...
import { fixture } from '../examples/demo.js';
...
test('real npm migration pins target, adapts API and preserves tests',async()=>{
  const f=await fixture();try{
    ...
    const report=await execute(f);assert.equal(report.status,'verified',report.error);
    ...
    assert.equal(JSON.parse(await fs.readFile(path.join(f.repo,'package.json'),'utf8')).dependencies['bumplab-fixture-name'],'1.0.0');
    assert.equal((await command(['git','apply','--check',path.join(f.out,'change.patch')],f.repo)).code,0);
    assert.equal(report.attempts.length,1);
  }finally{await f.close()}
});
test('refuses a dirty baseline and a fake upgrade',async()=>{
  const f=await fixture();try{
    const r=await execute({...f,config:{...f.config,update:[process.execPath,'-e','process.exit(0)']}});
    assert.equal(r.status,'blocked');assert.match(r.error,/target version/);
  }finally{await f.close()}
});
test('an agent cannot roll back the requested dependency',async()=>{
  const f=await fixture();try{
    const adapter=path.join(f.root,'cheat.cjs');await fs.writeFile(adapter,'process.stdin.resume();process.stdin.on("end",()=>console.log(JSON.stringify({summary:"rollback",edits:[{path:"package.json",content:"{}"}]})));');
    const r=await execute({...f,config:{...f.config,agent:[process.execPath,adapter]}});
    assert.equal(r.status,'blocked');assert.match(r.error,/outside source scope/);
  }finally{await f.close()}
});
